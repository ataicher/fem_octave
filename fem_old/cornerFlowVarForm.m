if SETDEGENERATE
    ...
    switch NAIVE
        ...
        case 3
            ...
            field(1).varForm.v{1} = @(u, gradU, x, y) (U_0/w_0)*u.field{1}(1) - (2+2*Theta)*phi(x,y)^(1+2*Theta)*dxPhi(x,y)*u.field{2}(1);
            field(1).varForm.v{2} = @(u, gradU, x, y) (U_0/w_0)*u.field{1}(2) - (2+2*Theta)*phi(x,y)^(1+2*Theta)*dyPhi(x,y)*u.field{2}(1);
        ...
        case 4
            % ( u_r , v_r ) - ( q_f , div v_r )
            field(1).varForm.v{1} = @(u, gradU, x, y) (U_0/w_0)*u.field{5}(1);
            field(1).varForm.v{2} = @(u, gradU, x, y) (U_0/w_0)*u.field{5}(2);
            field(1).varForm.gradV{1,1} = @(u, gradU, x, y) -u.field{2}(1);
            field(1).varForm.gradV{2,2} = @(u, gradU, x, y) -u.field{2}(1);
