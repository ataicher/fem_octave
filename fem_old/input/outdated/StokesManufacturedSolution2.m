...
x = [-1 0 1]; y = [-1 0 1];
loc = @(x,y) (x >=0) + (y >=0);
[nodes, edges, cells] = QuadMesh2(x,y,loc);
...
alpha = [0 0];
beta = [1 1];
field(1).bndry(5).loc = @(x,y) y;
field(1).bndry(5).alpha = alpha;
field(1).bndry(5).beta = beta;
field(1).bndry(5).eta = {@(x,y) alpha(1)*(n(1)*dxUx(x,y)*n(1) + n(1)*dyUx(x,y)*n(2) + n(2)*dxUy(x,y)*n(1) + n(2)*dyUy(x,y)*n(2) - P(x,y)) + ...
    beta(1)*(Ux(x,y)*n(1) + Uy(x,y)*n(2)), ... 
    @(x,y) alpha(2)*(tau(1)*dxUx(x,y)*n(1) + tau(1)*dyUx(x,y)*n(2) + tau(2)*dxUy(x,y)*n(1) + tau(2)*dyUy(x,y)*n(2)) + ...
    beta(2)*(Ux(x,y)*tau(1) + Uy(x,y)*tau(2))};
...
alpha = [0 0];
beta = [1 1];
field(1).bndry(6).loc = @(x,y) x;
field(1).bndry(6).alpha = alpha;
field(1).bndry(6).beta = beta;
field(1).bndry(6).eta = {@(x,y) alpha(1)*(n(1)*dxUx(x,y)*n(1) + n(1)*dyUx(x,y)*n(2) + n(2)*dxUy(x,y)*n(1) + n(2)*dyUy(x,y)*n(2) - P(x,y)) + ...
    beta(1)*(Ux(x,y)*n(1) + Uy(x,y)*n(2)), ... 
    @(x,y) alpha(2)*(tau(1)*dxUx(x,y)*n(1) + tau(1)*dyUx(x,y)*n(2) + tau(2)*dxUy(x,y)*n(1) + tau(2)*dyUy(x,y)*n(2)) + ...
    beta(2)*(Ux(x,y)*tau(1) + Uy(x,y)*tau(2))};
