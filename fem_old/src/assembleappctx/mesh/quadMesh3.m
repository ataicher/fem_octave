...
%          of node_i.
%
function [nodes, edges, cells] = QuadMesh3(x,y,varargin)
...
for j=1:ny
    for i=1:nx
        cPtr = (j-1)*nx + i;
        edgeBPtr = (nx+1)*ny + (j-1)*nx + i;
        edgeRPtr = (j-1)*(nx+1) + i + 1;
        edgeTPtr = (nx+1)*ny + j*nx + i;
        edgeLPtr = (j-1)*(nx+1) + i;
        cells(:,cPtr) = [edgeBPtr edgeRPtr -edgeTPtr -edgeLPtr]';
% check if cell is connected to a removed edge
        if sum((eRemovePtrs == edgeBPtr) + (eRemovePtrs == edgeRPtr) + ...
                (eRemovePtrs == edgeTPtr) + (eRemovePtrs == edgeLPtr)) > 0
            cReomvePtrs = [cReomvePtrs cPtr];
        end
    end
end

%% REMOVE NODES, EDGES, CELLS----------------------------------------------
% remove nodes
