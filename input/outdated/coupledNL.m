...
%
% Solve the PDE
%   -div((1+mu*|u2|^2)*gradU1) = f1
%   -div(gradU2) + (u1 - u2) = f2
% with Dirichlet boundary conditions
%
% for all v1 in V1
%    ((1 + mu*|u2|^2)*gradU1 , gradV1) - (f1 , v1) - <(1 + mu*|u2|^2)*gradU1.*n , v1> = 0
% for all v2 in  V2
...
uE.field{1}{1} = @(x,y) x*y*sin(pi*x);
uE.field{1}{2} = @(x,y) x*y;
uE.field{2}{1} = @(x,y) x*cos(pi*y*x);
...
f1x = @(x,y) -2*mu*( (U2x(x,y)*dxU2x(x,y) + U2y(x,y)*dxU2y(x,y))*dxU1x(x,y) + ...
        (U2x(x,y)*dyU2x(x,y) + U2y(x,y)*dyU2y(x,y))*dyU1x(x,y)) ...
        - (1 + mu*(U1x(x,y)^2 + U2x(x,y)^2))*(dxxU1x(x,y) + dyyU1x(x,y));
f1y = @(x,y) -2*mu*( (U2x(x,y)*dxU2x(x,y) + U2y(x,y)*dxU2y(x,y))*dxU1y(x,y) + ...
        (U2x(x,y)*dyU2x(x,y) + U2y(x,y)*dyU2y(x,y))*dyU1y(x,y)) ...
        - (1 + mu*(U1x(x,y)^2 + U2x(x,y)^2))*(dxxU1y(x,y) + dyyU1y(x,y));
...
varForm.field{1}.gradV{1,1} = @(u, gradU, x, y) (1 + mu*(u.field{2}(1)^2 + u.field{2}(2)^2))*gradU.field{1}(1,1);
varForm.field{1}.gradV{1,2} = @(u, gradU, x, y) (1 + mu*(u.field{2}(1)^2 + u.field{2}(2)^2))*gradU.field{1}(1,2);
varForm.field{1}.gradV{2,1} = @(u, gradU, x, y) (1 + mu*(u.field{2}(1)^2 + u.field{2}(2)^2))*gradU.field{1}(2,1);
varForm.field{1}.gradV{2,2} = @(u, gradU, x, y) (1 + mu*(u.field{2}(1)^2 + u.field{2}(2)^2))*gradU.field{1}(2,2);
% (u1 - u2 - f2 , v2)
