...
for c=1:numCells
    ...
    if h
        for fJ=1:numFields
		  ...
		  varFormVVal = zeros(numCompJ,1);
		  varFormGradVVal = zeros(numCompJ,dim);
		  for comp = 1:numCompJ
		    varFormVVal(comp) = varForm.v{comp}(uVal{q},gradUVal{q},realQuadPoints(1,q),realQuadPoints(2,q));
		    for d = 1:dim
		      varFormGradVVal(comp,d) = varForm.gradV{comp,d}(uVal{q},gradUVal{q},realQuadPoints(1,q),realQuadPoints(2,q));
		    end
		  end
%varFormVVal =  varForm.v(uVal{q},gradUVal{q},realQuadPoints(1,q),realQuadPoints(2,q));
 %                   varFormGradVVal = varForm.gradV(uVal{q},gradUVal{q},realQuadPoints(1,q),realQuadPoints(2,q));
		  ...
		  varFormVVal_h = zeros(numCompJ,1);
		  varFormGradVVal_h = zeros(numCompJ,dim);
		  for comp = 1:numCompJ
		    varFormVVal_h(comp) = varForm.v{comp}(uVal_h,gradUVal_h,realQuadPoints(1,q),realQuadPoints(2,q));
		    for d = 1:dim
		      varFormGradVVal_h(comp,d) = varForm.gradV{comp,d}(uVal_h,gradUVal_h,realQuadPoints(1,q),realQuadPoints(2,q));
		    end
		  end
