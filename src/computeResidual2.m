...
for c=1:numCells
    ...
    for f=1:numFields
	  ...
	  phiValCex(q) = phi(realQuadPoints(1,q),realQuadPoints(2,q));
	  end
	  ...
	  if 1
	    ...
	    if f == 1
	      ...
	      rBasisDer_xx = reshape(rBasis(:,1,1,:),numBasisFuncs,numQuadPoints);
	      rBasisDer_yy = reshape(rBasis(:,2,2,:),numBasisFuncs,numQuadPoints);
	      v_f_x = v_f(1,q);
	      v_f_y = v_f(2,q);
	      gradV_f_xx = gradV_f(1,1,q);
	      gradV_f_yy = gradV_f(2,2,q);
	      elemVec = sum(bsxfun(@times,rBasis1, quadWeights.*phiValVec.^(-2-2*Theta).*v_f_x),2);
	      elemVec = sum(bsxfun(@times,rBasis2, quadWeights.*phiValVec.^(-2-2*Theta).*v_f_y),2);		
	      elemVec = -sum(bsxfun(@times,rBasisDer_xx,quadWeights.*p_f),2);
	      elemVec = -sum(bsxfun(@times,rBasisDer_yy,quadWeights.*p_f),2);	      

	    elseif f ==2
	      rBasis = reshape(rBasis,1,numQuadPoints);
	      elemVec = -sum(bsxfun(@times,rBasis, quadWeights.*(gradV_f_xx + gradV_f_yy + phiValVec.*(p_f - p_m))),2);
            elseif f == 3
	      rBasis_x = reshape(rBasis(:,1,:),numBasisFuncs,numQuadPoints);
	      rBasis_y = reshape(rBasis(:,2,:),numBasisFuncs,numQuadPoints);
	      rBasisDer_xx = reshape(rBasis(:,1,1,:),numBasisFuncs,numQuadPoints);
	      rBasisDer_yy = reshape(rBasis(:,2,2,:),numBasisFuncs,numQuadPoints);
	      rBasisDer_xy = reshape(rBasis(:,1,2,:),numBasisFuncs,numQuadPoints);
	      rBasisDer_yx = reshape(rBasis(:,2,1,:),numBasisFuncs,numQuadPoints);
	      gradV_f_xx = gradV_f(1,1,q);
	      gradV_f_yy = gradV_f(2,2,q);
	      	      
	      elemVec = -sum(bsxfun(@times,rBasis_x, quadWeights.*(1-phiValVec)),2);
	                    
	      elemVec = sum(bsxfun(@times,rBasisDer_xx, quadWeights.*(2*(1-phiValVec) * v_m_xx - (5- 2 * phiValVec)/3 * (v_m_xx + v_m_yy) - p_m)),2);		  
	      elemVec = sum(bsxfun(@times,rBasisDer_yy, quadWeights.*(2*(1-phiValVec) * v_m_yy - (5- 2 * phiValVec)/3 * (v_m_xx + v_m_yy) - p_m)),2);

	      elemVec = sum(bsxfun(@times,*rBasisDer_xy, quadWeights.* ( (1-phiValVec) * (v_m_xx + v_m_yy))),2);
	      elemVec = sum(bsxfun(@times,*rBasisDer_yx, quadWeights.* ( (1-phiValVec) * (v_m_xx + v_m_yy))),2);
	    elseif f == 4
	      rBasis = reshape(rBasis,1,numQuadPoints);
	      elemVec = sum(bsxfun(@times, rBasis, quadWeights.*(-gradV_m_xx - gradV_m_yy + phiValVec .* (p_f - p_m))),2);
	      elemVec = -sum(bsxfun(@times,rBasis, quadWeights.*(gradV_f_xx + gradV_f_yy + phiValVec .* (p_f - p_m))),2);
